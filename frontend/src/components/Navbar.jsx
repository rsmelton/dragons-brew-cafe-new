...
const Navbar = () => {
    return (
        <Box className='sofia-regular' maxW={'full'} paddingTop={3} bgColor={'#3E3831'} color={'#FEEFB0'}>
            <Flex height={20} alignItems={'center'} justifyContent={'center'} gap={{base: '1rem', md: '2rem', lg: '3rem'}} flexDirection={'row'} paddingLeft={10}>
                {/* <Text></Text> */}
                <Text fontSize={{base: '20px', md: '25px', lg: '25px'}} _hover={{ color: '#3EA56C' }}><Link to={"/home"}>Home</Link></Text>
                <Image borderRadius={'full'} boxSize={'105px'} src={dragonsBrewCafeLogo} alt='Dragons Brew Cafe' />
                <Text fontSize={{base: '20px', md: '25px', lg: '25px'}} _hover={{ color: '#3EA56C' }}><Link to={"/menu"}>Menu</Link></Text>  
                <Text fontSize={{base: '20px', md: '25px', lg: '25px'}} _hover={{ color: '#3EA56C' }}><Link to={"/cart"}>Cart</Link></Text>         
            </Flex>
        </Box>
      )
